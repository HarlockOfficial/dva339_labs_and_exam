from Expression import NumberExpression, BooleanExpression, BinaryOperatorExpression, IdentifierExpression, \
    UnaryOperatorExpression, SeparatorExpression, BlockExpression, FunctionCallExpression, SingleExpression
from Statement import AssignmentStatement, SequenceStatement, FunctionDeclaration, IdentifierList, IfStatement, \
    WhileStatement, ReturnStatement, DefinitionStatement, BlockStatement
...
class MyParser(Parser):
    ...
    def Decl(self, p):
        return FunctionDeclaration(p[0], p[1], p[3], p[6])
    ...
    def FormalList(self, p):
        if len(p) == 0:
            return None
        if len(p) == 3:
            return IdentifierList(p[0], p[1], p[2])
        return IdentifierList(p[0], p[1])
    ...
    def Identifiers(self, p):
        if len(p) == 0:
            return None
        if len(p) == 4:
            return IdentifierList(p[1], p[2], p[3])
        return IdentifierList(p[1], p[2])
    ...
    def Stmt(self, p):
        return DefinitionStatement(p[0], p[1])
    ...
    def Expr(self, p):
        return BinaryOperatorExpression(BinaryOperatorType(p[1]), p[0], p[2])  # TODO check may not work
    ...
    def Expr(self, p):
        return FunctionCallExpression(p[0], BlockExpression(p[2]))
