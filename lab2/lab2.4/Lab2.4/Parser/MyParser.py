...
from Parser.Type import UnaryOperatorType, BinaryOperatorType
...
class MyParser(Parser):
    ...
    def Decl(self, p):
        return FunctionDeclaration(p.lineno, p.index, p[0], IdentifierExpression(p.lineno, p.index, p[1]), p[3], p[6])
    ...
    def FormalList(self, p):
        if len(p) == 0:
            return None
        if len(p) == 3:
            return IdentifierList(p.lineno, p.index, p[0], IdentifierExpression(p.lineno, p.index, p[1]), p[2])
        return IdentifierList(p.lineno, p.index, p[0], IdentifierExpression(p.lineno, p.index, p[1]))
    ...
    def Identifiers(self, p):
        if len(p) == 0:
            return None
        if len(p) == 4:
            return IdentifierList(p.lineno, p.index, p[1], IdentifierExpression(p.lineno, p.index, p[2]), p[3])
        return IdentifierList(p.lineno, p.index, p[1], IdentifierExpression(p.lineno, p.index, p[2]))
    ...
    def Stmt(self, p):
        return BlockStatement(p.lineno, p.index, p[1])
    ...
    @_('Expr SEMICOLON')
    def Stmt(self, p):
        return SingleExpression(p.lineno, p.index, p[0])

    @_('Type IDENTIFIER SEMICOLON')
    def Stmt(self, p):
        return DefinitionStatement(p.lineno, p.index, p[0], IdentifierExpression(p.lineno, p.index, p[1]))

    @_('RETURN Expr SEMICOLON', 'RETURN SEMICOLON')
    def Stmt(self, p):
        if len(p) == 3:
            return ReturnStatement(p.lineno, p.index, p[1])
        return ReturnStatement(p.lineno, p.index)
    ...
    def Stmt(self, p):
        if len(p) == 7:
            return IfStatement(p.lineno, p.index, p[2], p[4], p[6])
        return IfStatement(p.lineno, p.index, p[2], p[4])

    @_('WHILE LEFTROUNDPARENTHESES Expr RIGHTROUNDPARENTHESES Stmt')
    def Stmt(self, p):
        return WhileStatement(p.lineno, p.index, p[2], p[4])
    ...
    @_('NUMBER')
    def Expr(self, p):
        return NumberExpression(p.lineno, p.index, p[0])

    @_('TRUE', 'FALSE')
    def Expr(self, p):
        return BooleanExpression(p.lineno, p.index, p[0])

    @_('IDENTIFIER ASSIGN Expr')
    def Expr(self, p):
        return AssignmentStatement(p.lineno, p.index, IdentifierExpression(p.lineno, p.index, p[0]), p[2])
    ...
    def Expr(self, p):
        return BinaryOperatorExpression(p.lineno, p.index, BinaryOperatorType(p[1]), p[0], p[2])

    @_('NOT Expr', 'MINUS Expr %prec UMINUS')
    def Expr(self, p):
        return UnaryOperatorExpression(p.lineno, p.index, UnaryOperatorType(p[0]), p[1])

    @_('IDENTIFIER LEFTROUNDPARENTHESES ExprList RIGHTROUNDPARENTHESES')
    def Expr(self, p):
        return FunctionCallExpression(p.lineno, p.index, IdentifierExpression(p.lineno, p.index, p[0]),
                                      BlockExpression(p.lineno, p.index, p[2]))

    @_('IDENTIFIER')
    def Expr(self, p):
        return IdentifierExpression(p.lineno, p.index, p[0])

    @_('LEFTROUNDPARENTHESES Expr RIGHTROUNDPARENTHESES')
    def Expr(self, p):
        return BlockExpression(p.lineno, p.index, p[1])
