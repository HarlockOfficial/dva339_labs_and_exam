...
class SequenceStatement(Statement):
    ...
    def typecheck(self, state: "TypeChecker.TypecheckingState" = None) -> \
            Union[Tuple[bool, "TypeChecker.TypecheckingState"], List[Union["TypeChecker.Types", Any]],
                  "TypeChecker.Types"]:
        if state is None:
...
class WhileStatement(Statement):
    def __init__(self, line: int, column: int, expression: "AbstractSyntax.Expression", body: Statement):
        super().__init__(line, column)
        self.__expression = expression
        self.__body = body
...
class DefinitionStatement(Statement):
    ...
    def compile(self, state: "Generator.GeneratorState", program: "Generator.Trac42Program"):
        ...
        program.emit(Generator.Instruction(Generator.OpCode.DECL, 1))  # declaring variable
